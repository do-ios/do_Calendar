...
@interface DOCalendar (DataSourceAndDelegate)
...
- (DOCalendarCellShape)preferredCellShapeForDate:(NSDate *)date;
...
@interface DOCalendar ()<UICollectionViewDataSource, UICollectionViewDelegate>
{
...
@property (strong, nonatomic) DOCalendarAnimator         *animator;

@property (weak  , nonatomic) DOCalendarHeader           *header;
@property (weak  , nonatomic) DOCalendarHeaderTouchDeliver *deliver;
...
@property (readonly, nonatomic) id<DOCalendarDelegateAppearance> delegateAppearance;
...
- (void)reloadDataForCell:(DOCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath;
- (void)reloadVisibleCells;

@end

@implementation DOCalendar
...
- (void)initialize
{
    _appearance = [[DOCalendarAppearance alloc] init];
    ...
    _headerHeight     = DOCalendarAutomaticDimension;
    _weekdayHeight    = DOCalendarAutomaticDimension;
    
    _preferredHeaderHeight  = DOCalendarAutomaticDimension;
    _preferredWeekdayHeight = DOCalendarAutomaticDimension;
    _preferredRowHeight     = DOCalendarAutomaticDimension;
    _lineHeightMultiplier    = 1.0;
    
    _scrollDirection = DOCalendarScrollDirectionHorizontal;
    _scope = DOCalendarScopeMonth;
    ...
    DOCalendarFlowLayout *collectionViewLayout = [[DOCalendarFlowLayout alloc] init];
    ...
    [collectionView registerClass:[DOCalendarCell class] forCellWithReuseIdentifier:@"cell"];
    [collectionView registerClass:[DOCalendarStickyHeader class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header"];
    ...
    self.animator = [[DOCalendarAnimator alloc] init];
...
- (void)setBounds:(CGRect)bounds
{
    ...
    if (!CGRectIsEmpty(bounds) && self.animator.state == DOCalendarTransitionStateIdle) {
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (!CGRectIsEmpty(frame) && self.animator.state == DOCalendarTransitionStateIdle) {
        [self invalidateViewFrames];
    }
}
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        BOOL needsAdjustingBoundingRect = self.scope == DOCalendarScopeMonth && !_showsPlaceholders && !self.hasValidateVisibleLayout;
        
        _contentView.frame = self.bounds;
        
        if (_needsLayoutForWeekMode) _scope = DOCalendarScopeMonth;
        ...
        if (_needsLayoutForWeekMode) _scope = DOCalendarScopeWeek;
            ...
            switch (_scope) {
                case DOCalendarScopeMonth: {
                    ...
                    if (needsAdjustingBoundingRect) {
                        self.animator.state = DOCalendarTransitionStateInProgress;
                        [self boundingRectWillChange:NO];
                        self.animator.state = DOCalendarTransitionStateIdle;
                    }
                    break;
                }
    ...
    if (_needsLayoutForWeekMode) {
        ...
        _scope = DOCalendarScopeWeek;
        [self.animator performScopeTransitionFromScope:DOCalendarScopeMonth toScope:DOCalendarScopeWeek animated:NO];
    } else {
...
- (CGSize)sizeThatFits:(CGSize)size
{
    switch (self.animator.transition) {
        case DOCalendarTransitionStateIdle:
            return [self sizeThatFits:size scope:_scope];
        case DOCalendarTransitionWeekToMonth:
            if (self.animator.state == DOCalendarTransitionStateInProgress) {
                return [self sizeThatFits:size scope:DOCalendarScopeMonth];
            }
        case DOCalendarTransitionMonthToWeek:
            break;
    }
    return [self sizeThatFits:size scope:DOCalendarScopeWeek];
}
...
{
    if (self.animator.transition == DOCalendarTransitionWeekToMonth) {
...
{
    if (self.animator.transition == DOCalendarTransitionWeekToMonth && self.animator.state == DOCalendarTransitionStateInProgress) {
        return 42;
    }
    if (!self.floatingMode) {
        switch (_scope) {
...
{
    DOCalendarCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (self.floatingMode) {
        if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
            DOCalendarStickyHeader *stickyHeader = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header" forIndexPath:indexPath];
...
{
    DOCalendarCell *cell = (DOCalendarCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
{
    if ([elementKind isEqualToString:UICollectionElementKindSectionHeader] && [view isKindOfClass:[DOCalendarStickyHeader class]]) {
        [view setNeedsLayout];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.floatingMode && _collectionView.indexPathsForVisibleItems.count) {
        ...
        if (significantIndexPath) {
            ...
            __block DOCalendarStickyHeader *significantHeader = nil;
            [_stickyHeaderMapTable.dictionaryRepresentation.allValues enumerateObjectsUsingBlock:^(DOCalendarStickyHeader *header, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if (shouldTriggerPageChange) {
        ...
        [self currentPageDidChange];
        if (!_showsPlaceholders && self.animator.state == DOCalendarTransitionStateIdle) {
...
- (void)setAppearance:(DOCalendarAppearance *)appearance
{
...
- (DOCalendarAppearance *)appearance
{
    return _appearance;
}

- (void)setScrollDirection:(DOCalendarScrollDirection)scrollDirection
{
...
- (void)setScope:(DOCalendarScope)scope
{
...
- (void)setDataSource:(id<DOCalendarDataSource>)dataSource
{
...
{
    if (_headerHeight == DOCalendarAutomaticDimension) {
        if (_preferredWeekdayHeight == DOCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                CGFloat divider = _scope == DOCalendarScopeMonth ? DOCalendarStandardMonthlyPageHeight : DOCalendarStandardWeeklyPageHeight;
                _preferredHeaderHeight = (DOCalendarStandardHeaderHeight/divider)*self.fs_height;
                _preferredHeaderHeight -= (_preferredHeaderHeight-DOCalendarStandardHeaderHeight)*0.5;
            } else {
                _preferredHeaderHeight = DOCalendarStandardHeaderHeight*MAX(1, FSCalendarDeviceIsIPad*1.5)*_lineHeightMultiplier;
            }
        }
...
{
    if (_weekdayHeight == DOCalendarAutomaticDimension) {
        if (_preferredWeekdayHeight == DOCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                CGFloat divider = _scope == DOCalendarScopeMonth ? DOCalendarStandardMonthlyPageHeight : DOCalendarStandardWeeklyPageHeight;
                _preferredWeekdayHeight = (DOCalendarStandardWeekdayHeight/divider)*self.fs_height;
            } else {
                _preferredWeekdayHeight = DOCalendarStandardWeekdayHeight*MAX(1, FSCalendarDeviceIsIPad*1.5)*_lineHeightMultiplier;
            }
        }
...
{
    if (_preferredRowHeight == DOCalendarAutomaticDimension) {
        CGFloat headerHeight = self.preferredHeaderHeight;
        CGFloat weekdayHeight = self.preferredWeekdayHeight;
        ...
        if (!self.floatingMode) {
            switch (_scope) {
                case DOCalendarScopeMonth: {
                    _preferredRowHeight = _showsPlaceholders ? (contentHeight-padding*2)/6.0 : DOCalendarStandardRowHeight;
                    break;
                }
                case DOCalendarScopeWeek: {
                    _preferredRowHeight = _showsPlaceholders ? contentHeight-padding*2 : DOCalendarStandardRowHeight;
                    break;
                }
            }
        } else {
            _preferredRowHeight = DOCalendarStandardRowHeight*MAX(1, FSCalendarDeviceIsIPad*1.5)*_lineHeightMultiplier;
        }
    }
    return _preferredRowHeight;
}

- (id<DOCalendarDelegateAppearance>)delegateAppearance
{
    if (_delegate && [_delegate conformsToProtocol:@protocol(DOCalendarDelegateAppearance)]) {
        return (id<DOCalendarDelegateAppearance>)_delegate;
    }
    return nil;
}
...
- (void)setScope:(DOCalendarScope)scope animated:(BOOL)animated
{
    if (_scope != scope) {
...
[self didChangeValueForKey:@"scope"]; \
        ...
        DOCalendarScope prevScope = _scope;
        
        if (!self.hasValidateVisibleLayout && prevScope == DOCalendarScopeMonth && scope == DOCalendarScopeWeek) {
            m_set_scope
        ...
        if (self.animator.state == DOCalendarTransitionStateIdle) {
            m_set_scope
...
- (void)deselectDate:(NSDate *)date
{
    ...
    if ([_collectionView.indexPathsForSelectedItems containsObject:indexPath]) {
        ...
        DOCalendarCell *cell = (DOCalendarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        cell.dateIsSelected = NO;
        [cell setNeedsLayout];
...
- (void)selectDate:(NSDate *)date scrollToDate:(BOOL)scrollToDate forPlaceholder:(BOOL)forPlaceholder
{
    ...
    if (forPlaceholder) {
        ...
        [_collectionView selectItemAtIndexPath:targetIndexPath animated:YES scrollPosition:UICollectionViewScrollPositionNone];
        DOCalendarCell *cell = (DOCalendarCell *)[_collectionView cellForItemAtIndexPath:targetIndexPath];
        [cell performSelecting];
...
{
    if (self.animator.transition == DOCalendarTransitionWeekToMonth && self.animator.state == DOCalendarTransitionStateInProgress) {
        return [self dateForIndexPath:indexPath scope:DOCalendarScopeMonth];
    }
...
- (void)invalidateLayout
{
    if (!self.floatingMode) {
        ...
        if (!_header) {
            ...
            DOCalendarHeader *header = [[DOCalendarHeader alloc] initWithFrame:CGRectZero];
        ...
        if (_scrollEnabled) {
            if (!_deliver) {
                DOCalendarHeaderTouchDeliver *deliver = [[DOCalendarHeaderTouchDeliver alloc] initWithFrame:CGRectZero];
...
- (void)invalidateWeekdaySymbols
{
    BOOL useVeryShortWeekdaySymbols = (_appearance.caseOptions & (15<<4) ) == DOCalendarCaseOptionsWeekdayUsesSingleUpperCase;
    ...
    BOOL useDefaultWeekdayCase = (_appearance.caseOptions & (15<<4) ) == DOCalendarCaseOptionsWeekdayUsesDefaultCase;
...
{
    if (!self.floatingMode) {
        [_collectionView.visibleCells enumerateObjectsUsingBlock:^(DOCalendarCell *cell, NSUInteger idx, BOOL *stop) {
...
{
    if (self.floatingMode) {
        [_collectionView.visibleCells enumerateObjectsUsingBlock:^(DOCalendarCell *cell, NSUInteger index, BOOL *stop) {
...
- (DOCalendarCellShape)preferredCellShapeForDate:(NSDate *)date
{
    if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:cellShapeForDate:)]) {
        DOCalendarCellShape cellShape = [self.delegateAppearance calendar:self appearance:self.appearance cellShapeForDate:date];
        return cellShape;
    }
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    else if (self.delegateAppearance && [self.delegateAppearance respondsToSelector:@selector(calendar:appearance:cellStyleForDate:)]) {
        DOCalendarCellShape cellShape = (DOCalendarCellShape)[self.delegateAppearance calendar:self appearance:self.appearance cellStyleForDate:date];
        return cellShape;
    }
#pragma GCC diagnostic pop
    
    return DOCalendarCellShapeCircle;
}
...
@implementation DOCalendar (DateTools)
...
- (NSDate *)dateByIgnoringTimeComponentsOfDate:(NSDate *)date
{
    ...
    components.hour = DOCalendarDefaultHourComponent;
...
@implementation DOCalendar (Deprecated)
...
- (void)setFlow:(DOCalendarFlow)flow
{
    self.scrollDirection = (DOCalendarScrollDirection)flow;
}

- (DOCalendarFlow)flow
{
    return (DOCalendarFlow)self.scrollDirection;
}
