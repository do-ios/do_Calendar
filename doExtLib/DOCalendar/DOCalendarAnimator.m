...
@implementation DOCalendarAnimator
...
- (void)performScopeTransitionFromScope:(DOCalendarScope)fromScope toScope:(DOCalendarScope)toScope animated:(BOOL)animated
{
    if (fromScope == toScope) {
        self.transition = DOCalendarTransitionNone;
        return;
    }
    if (fromScope == DOCalendarScopeMonth && toScope == DOCalendarScopeWeek) {
        self.transition = DOCalendarTransitionMonthToWeek;
    } else if (fromScope == DOCalendarScopeWeek && toScope == DOCalendarScopeMonth) {
        self.transition = DOCalendarTransitionWeekToMonth;
    }
    
    // Start transition
    self.state = DOCalendarTransitionStateInProgress;
    ...
    switch (self.transition) {
        ...
        case DOCalendarTransitionMonthToWeek: {
            ...
            CGSize contentSize = [self.calendar sizeThatFits:self.calendar.frame.size scope:DOCalendarScopeWeek];
            ...
            if (self.calendar.focusOnSingleSelectedDate) {
                ...
                if (focusedDate) {
                    UICollectionViewLayoutAttributes *attributes = [self.collectionViewLayout layoutAttributesForItemAtIndexPath:[self.calendar indexPathForDate:focusedDate scope:DOCalendarScopeMonth]];
            ...
            NSDate *firstDate = [self.calendar dateForIndexPath:firstIndexPath scope:DOCalendarScopeMonth];
            currentPage = [self.calendar dateByAddingDays:focusedRowNumber*7 toDate:firstDate];
            
            Ivar currentPageIvar = class_getInstanceVariable(DOCalendar.class, "_currentPage");
            ...
            if (animated) {
                ...
                opacity.toValue = @0;
                [self.collectionView.visibleCells enumerateObjectsUsingBlock:^(DOCalendarCell *cell, NSUInteger idx, BOOL *stop) {
                ...
                [CATransaction setCompletionBlock:^{
                    self.state = DOCalendarTransitionStateIdle;
                    self.transition = DOCalendarTransitionNone;
...
#pragma GCC diagnostic pop
        ...
        case DOCalendarTransitionWeekToMonth: {
            ...
            if (CGSizeEqualToSize(CGSizeZero, contentSize)) {
                contentSize = [self.calendar sizeThatFits:self.calendar.frame.size scope:DOCalendarScopeMonth];
            }
            ...
            if (self.calendar.focusOnSingleSelectedDate) {
                NSDate *focusedDate = self.calendar.selectedDate;
                if (focusedDate) {
                    UICollectionViewLayoutAttributes *attributes = [self.collectionViewLayout layoutAttributesForItemAtIndexPath:[self.calendar indexPathForDate:focusedDate scope:DOCalendarScopeWeek]];
                    CGPoint focuedCenter = attributes.center;
                    if (CGRectContainsPoint(self.collectionView.bounds, focuedCenter)) {
